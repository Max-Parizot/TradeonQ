...
def get_states(Data, n, N):
    ...
    rho = rho[dS:]  #This grabs only the most recent data
    # Price changes
    DS=np.zeros_like(Data['S']) 
    for i in range(dS,len(Data['S'])):  #dS,check to make sure that data is being input correctly
        DS[i] = np.sign(Data['S'].iloc[i] - Data['S'].iloc[i - dS])
    DS = DS[dS:]  #this grabs only the most recent data from 
    return rho, DS
...
def trade_on_Q_with_cash_limit(Data,Q,n,N,cash,prob_limit):
    ...
    for tt in range(1, T-N): # added the additional -1
        ...
        # If predicting a downward price move
        if Q[row, downColumn] > .5+ prob_limit:
            if assets>=1:
                cash = cash + MOBid[tt] # Sell
                assets = assets - 1
            elif -(assets-1)*MOAsk[tt]<cash + (assets-1) *MOAsk[tt]:
                cash = cash + MOBid[tt]
                assets = assets - 1

        # If predicting an upward price move
        elif Q[row, upColumn] > .5+ prob_limit:
            if cash>MOAsk[tt]:
                cash = cash - MOAsk[tt] # Buy
                assets = assets + 1
    ...
    CASH[T-N+1] = cash
    ASSETS[T-N+1] = assets
    POSITIONS[T-N+1]=cash+assets*MOBid[tt]
    return cash
...
def update_plot(x, y,x_list,y_list):
